 ...
 * @param {HotspotPannellumHandlers} [handlers]
 * @returns {Array<{ id: string, pitch: number, yaw: number, type: string, text?: string, clickHandlerFunc?: function }>}
 */
export function mapHotspotsToPannellum(hotspots, handlers = {}) {
  const { onInfoClick, onSceneClick, getSceneLabel } = handlers;
  ...
  return hotspots.map((hs) => {
    ...
    const config = {
      ...
      text: hs.title || "Hotspot",
    };

    if (onInfoClick && hs.type === HOTSPOT_TYPE_INFO) {
